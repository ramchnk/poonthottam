...
import { subscribeToCollection, saveSalesmanPurchase, deleteSalesmanPurchase, updateData } from '../utils/storage';
import { LangContext } from '../components/Layout';
import { Plus, Trash2, Calendar, User, ShoppingBag, History, Clock, Pencil } from 'lucide-react';
...
const SalesmanPurchases = () => {
    ...
    const handleEditBill = (bill) => {
        ...
        setBillItems(bill.items.map((item, idx) => {
            const selectedFlower = flowers.find(f => f.name === item.flowerType);
            const flowerNameLocalized = lang === 'ta' ? (selectedFlower?.taName || item.flowerType) : item.flowerType;
            return {
                id: idx,
    ...
    const handleSubmitBill = async () => {
        ...
        try {
            ...
            if (editingBillId) {
                await updateData('salesman_purchases', editingBillId, billData);
                setEditingBillId(null);
                    ...
                    <div style={{ display: 'flex', justifySpaceBetween: 'space-between', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                            ...
                            <div style={{ display: 'flex', gap: '10px' }}>
                                ...
                                >
                                    {isSaving ? 'Saving...' : (editingBillId ? 'Update Bill' : 'Submit Bill')}
                                </button>
                                {editingBillId && (
                                    <button
                                        ...
                                        style={{
                                            ...
                                            background: '#f1f5f9', color: '#64748b', fontWeight: 700, fontSize: '14px',
                                            cursor: 'pointer'
                                        }}
                                    >
                                        Cancel
